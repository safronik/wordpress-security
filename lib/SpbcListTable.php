...
class SpbcListTable{
	...
	public $args = array(); // Itin arguments
	...
	public $func_data_get     = null; // Function to recive data
	...
	public function get_data(){
		...
		if($this->func_data_get && function_exists($this->func_data_get)){
			$this->rows = call_user_func_array($this->func_data_get, array($this->sql['offset'], $this->sql['limit']));
		}else{
			$this->rows = $wpdb->get_results(
				sprintf(
					'SELECT %s'
						...
						.' ORDER BY %s %s'
						.' LIMIT %s%d',
					implode(', ', $this->columns_names),            // COLUMNS
					$this->sql['table'],                            // TABLE
					$this->sql['where'],                            // WHERE
	...
	public function display__column_headers(){
		...
		foreach($this->columns as $column_key => $column){
			...
			if(in_array($column_key, $this->sortable)){
				...
				$sort_classes .=  isset($this->order_by[$column_key]) ? " icon-sort--{$sort_direction}" : ' icon-sort--desc';
			...
			echo "<$tag id='$id' class='$classes' $sort_direction_attr>{$column['heading']}$sortable</$tag>";
	...
	public static function ajax__row_action_handler(){
		...
		if(!empty($_POST['function'])){
			...
			switch($_POST['add_action']){
				...
				default: die('UNKNOWN ACTION'); break;
			}
		}
	}
	...
	{
		$result = spbc_scanner_file_approve(true, $_POST['id']);
	...
	{
		$result = spbc_scanner_file_delete(true, $_POST['id']);
		if(empty($result['error'])){
			$colspan = isset($_POST['cols']) ? "colspan='{$_POST['cols']}'" : '';
			$out = array(
	...
	{
		$result = spbc_scanner_file_replace(true, $_POST['id']);
		if(empty($result['error'])){
			$colspan = isset($_POST['cols']) ? "colspan='{$_POST['cols']}'" : '';
			$out = array(
	...
	{
		$result = spbc_scanner_file_send(true, $_POST['id']);
		if(empty($result['error'])){
			$colspan = isset($_POST['cols']) ? "colspan='{$_POST['cols']}'" : '';
			$out = array(
				'temp_html' => "<td $colspan>File has been send to CleanTalk cloud. Now you can try to delete the file, but you have to be sure that you site backup before delete any file.</td>",
				'success' => true,
				'color' => 'black',
	...
	{
		check_ajax_referer('spbc_secret_nonce', 'security');
		$order_by = substr($_POST['order_by'], 0, 15);
